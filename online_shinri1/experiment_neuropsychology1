...
// データに保存
jsPsych.data.addProperties({ level: level });
...
createBlocks(nbackStimuli.stimuliListThirdBlock, nbackStimuli.stimuliThirdBlock, level)
...
const thirdBlock = { ... firstBlock, timeline_variables: nbackStimuli.stimulithirdBlock }
...
jsPsych.data.addProperties({level: level});
timeline.push({type: "fullscreen", fullscreen_mode: true}, instructions, startPractice, practice, afterPractice, firstBlock, betweenBlockRest, ready, secondBlock, betweenBlockRest, ready, thirdBlock,debriefBlock, {type: "fullscreen", fullscreen_mode: false});
